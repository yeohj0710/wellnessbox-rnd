...
- **감사·봉인:** 블라인드 패킷과 AI 응답은 SHA-256으로 묶는다. 같은 계열 AI, 누락 사례, 엔진 출력 열람, 빠진 블라인딩 경로, 변조된 일괄 승인, 상세 검토로 위장한 일괄 결정을 차단한다. KPI-4 봉인은 상담 모델 제공자 계열을 필수로 받고 1차 AI와 같으면 거부한다.
...
- **주요 파일:** `adaptive_answer_key_review.py`, `answer_key_workbench.py`, `answer_key_integrity.py`, 두 봉인 CLI, `data/original_plan/kpi/ai_review_packets/`, 계약·KPI 전략·관련 테스트다. 엔진·안전 규칙·채점 로직·frozen eval·replay·slice는 바꾸지 않았다.
- **검증:** 관련 기능·봉인·완료 게이트 테스트 171건 PASS. 전체 pytest는 1,499건 중 기존과 같은 90건 실패, 전체 Ruff는 기존과 같은 28건으로 신규 실패·오류 0건이다. 정답지 출처 감사는 4/4 PASS이며 완료 상태는 사람 결정·봉인이 없어서 BLOCKED다.
- **다음 행동:** 사람이 KPI-1·5 봉인을 정식 폐기하고, 독립 Claude/Codex 응답을 패킷만 사용해 만든 뒤, `review-minimal`과 `approve-consensus`를 직접 수행한다. 2차년도 검토는 예비 약사 사전 검토로만 기록한다.
