...
def recommend(request: RecommendationRequest) -> RecommendationResponse:
    ...
    if safety_summary.status == RecommendationStatus.BLOCKED:
        ...
        if _can_clear_pregnancy_survey_only_block(
            ...
            recommendations = select_recommendations(intake, effective_safety_summary)
        else:
...
def _can_start_with_current_regimen_coverage(
    ...
    if renal_only:
        ...
        if not rule_ids.issubset({"SAFETY-RENAL-001", "SAFETY-DUP-001"}):
            return False
    elif not pregnancy_duplicate_glucose_regimen and rule_ids != {"SAFETY-DUP-001"}:
        return False
    ...
    if not current_items:
        return False
    if (
        not statin_duplicate_heart_regimen
        and any(
