...
def build_non_cgm_threshold_duration_sensitive_mid_margin_diagnostic(
    *,
    ...
    expected_case_count = int(
        _as_dict(narrowing_decision.get("decision_gate")).get(
            "chosen_first_margin_bucket_case_count",
            0,
        )
    )
    workflow_summary = _build_workflow_summary(bucket_cases)
    case_summary = _build_case_summary(
        bucket_cases=bucket_cases,
        parent_case_count=len(parent_cases),
    )
    feature_summary = _build_feature_summary(bucket_cases)
    ...
    diagnostic = {
        "audit_name": "non_cgm_threshold_duration_sensitive_mid_margin_diagnostic_v1",
        "source_artifacts": {
        ...
        "bucket_target": {
            ...
            "observed_case_count": len(bucket_cases),
        ...
        "example_cases": bucket_cases[:6],
    }
    diagnostic["validation_issues"] = (
        validate_non_cgm_threshold_duration_sensitive_mid_margin_diagnostic(diagnostic)
    )
    return diagnostic


def validate_non_cgm_threshold_duration_sensitive_mid_margin_diagnostic(
    diagnostic: dict[str, object]
) -> list[str]:
    issues: list[str] = []
    bucket_target = _as_dict(diagnostic.get("bucket_target"))
    ...
    expected_case_count = int(
        bucket_target.get("expected_case_count_from_narrowing_decision", 0)
    )
    observed_case_count = int(bucket_target.get("observed_case_count", 0))
    if expected_case_count != observed_case_count:
        issues.append("narrowing_decision_case_count_mismatch")
    if bucket_target.get("trajectory_mode") != TARGET_TRAJECTORY_MODE:
        issues.append("unexpected_trajectory_mode")
    if bucket_target.get("margin_bucket") != TARGET_MARGIN_BUCKET:
...
def render_non_cgm_threshold_duration_sensitive_mid_margin_diagnostic_markdown(
    diagnostic: dict[str, object]
) -> str:
    lines = [
        "# non-cgm threshold-duration-sensitive mid-margin diagnostic v1",
        "",
        "## readable summary",
        "",
        ...
        "## bucket target",
        "",
        f"- bucket_target: `{diagnostic.get('bucket_target', {})}`",
        "",
        "## workflow summary",
        "",
...
def write_non_cgm_threshold_duration_sensitive_mid_margin_diagnostic_files(
    *,
    diagnostic: dict[str, object],
    json_path: str | Path,
    md_path: str | Path,
...
def _build_workflow_summary(bucket_cases: list[dict[str, object]]) -> dict[str, object]:
    final_cycle_counts = Counter(str(case["final_cycle_index"]) for case in bucket_cases)
    proxy_drop_bucket_counts = Counter(str(case["proxy_drop_bucket"]) for case in bucket_cases)
    return {
...
def _build_case_summary(
    *,
    bucket_cases: list[dict[str, object]],
    parent_case_count: int,
) -> dict[str, object]:
    reference_margins = [float(case["reference_continue_margin"]) for case in bucket_cases]
    candidate_shortfalls = [float(case["candidate_monitor_shortfall"]) for case in bucket_cases]
    proxy_drops = [float(case["proxy_drop"]) for case in bucket_cases]
    share_pct = round((len(bucket_cases) / parent_case_count) * 100.0, 2)
    return {
        "all_cases_match_target_mode": all(
            str(case["trajectory_mode"]) == TARGET_TRAJECTORY_MODE for case in bucket_cases
        ),
        "all_cases_match_target_bucket": all(
            str(case["margin_bucket"]) == TARGET_MARGIN_BUCKET for case in bucket_cases
        ),
        "all_cases_non_cgm": all(case["cgm_available"] is False for case in bucket_cases),
...
def _build_feature_summary(bucket_cases: list[dict[str, object]]) -> dict[str, object]:
...
def _build_interpretation(
    *,
    ...
    return {
        ...
        "summary": (
            "The densest replay-only target inside `threshold_duration_sensitive` "
            "remains the `mid_margin` bucket, with the same intercept-led signature."
        ),
    }
...
def _build_summary_findings(readable_summary: dict[str, object]) -> list[str]:
    ...
    return [
        (
            "The current densest replay-only bucket remains "
            f"`{TARGET_MARGIN_BUCKET}` with "
            f"{case_digest.get('observed_case_count')}/"
            f"{case_digest.get('parent_family_case_count')} parent-family cases."
        ),
        (
            "Its current proxy-drop buckets are "
            f"{margin_digest.get('proxy_drop_bucket_counts')}."
        ),
        (
...
__all__ = [
    ...
    "load_json_artifact",
    "render_non_cgm_threshold_duration_sensitive_mid_margin_diagnostic_markdown",
    "validate_non_cgm_threshold_duration_sensitive_mid_margin_diagnostic",
    "write_non_cgm_threshold_duration_sensitive_mid_margin_diagnostic_files",
]
