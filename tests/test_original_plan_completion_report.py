...
def test_current_report_covers_all_requirements_without_inflating_completion() -> None:
    ...
    assert report.disposition_counts == {
        CompletionDisposition.COMPLETE: 30,
        CompletionDisposition.PARTIAL: 11,
        CompletionDisposition.PENDING: 78,
...
def test_global_source_failure_invalidates_every_existing_completion_claim() -> None:
    ...
    assert report.disposition_counts[CompletionDisposition.CONTRADICTED] == 41
...
def test_markdown_uses_audited_korean_status_language() -> None:
    ...
    assert "| 완료 | 30 |" in markdown
    assert "| 부분 완료 | 11 |" in markdown
    assert "| 대기 | 78 |" in markdown
...
def test_report_cli_writes_and_checks_deterministic_artifacts(tmp_path: Path) -> None:
    ...
    assert json.loads(generated.stdout)["disposition_counts"]["COMPLETE"] == 30
