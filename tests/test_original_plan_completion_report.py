...
def test_current_report_covers_all_requirements_without_inflating_completion() -> None:
    ...
    assert report.disposition_counts == {
        CompletionDisposition.COMPLETE: 72,
        CompletionDisposition.PARTIAL: 39,
        CompletionDisposition.PENDING: 8,
    ...
    assert _completion_by_id(report, "OP-094").disposition == CompletionDisposition.COMPLETE
    assert _completion_by_id(report, "OP-095").disposition == CompletionDisposition.COMPLETE
    assert _completion_by_id(report, "OP-096").disposition == CompletionDisposition.PARTIAL
    assert _completion_by_id(report, "OP-097").disposition == CompletionDisposition.COMPLETE
    assert _completion_by_id(report, "OP-098").disposition == CompletionDisposition.PARTIAL
    assert _completion_by_id(report, "OP-099").disposition == CompletionDisposition.PARTIAL
    assert _completion_by_id(report, "OP-100").disposition == CompletionDisposition.COMPLETE
    assert _completion_by_id(report, "OP-101").disposition == CompletionDisposition.PARTIAL
...
def test_global_source_failure_invalidates_every_existing_completion_claim() -> None:
    ...
    assert report.disposition_counts[CompletionDisposition.CONTRADICTED] == 111
...
def test_markdown_uses_audited_korean_status_language() -> None:
    ...
    assert "| 완료 | 72 |" in markdown
    assert "| 부분 완료 | 39 |" in markdown
    assert "| 대기 | 8 |" in markdown
...
def test_report_cli_writes_and_checks_deterministic_artifacts(tmp_path: Path) -> None:
    ...
    assert json.loads(generated.stdout)["disposition_counts"]["COMPLETE"] == 72
