...
def build_parser() -> ArgumentParser:
    parser = ArgumentParser(
        description="Generate deterministic rich synthetic longitudinal v3 cohort"
    )
    parser.add_argument("--seed", type=int, default=DEFAULT_RICH_SYNTHETIC_SEED + 1)
    ...
    parser.add_argument(
        ...
        help="JSONL output path for the v3 rich longitudinal cohort",
    )
    parser.add_argument(
        "--policy-output",
        default="data/synthetic/policy_training_v1_from_v3.jsonl",
        help="Optional JSONL output path for the derived policy dataset",
    )
    parser.add_argument(
        "--cohort-report-json",
        default="artifacts/reports/synthetic_longitudinal_v3_summary.json",
        help="Summary report JSON path for the v3 cohort",
    )
    parser.add_argument(
        "--cohort-report-md",
        default="artifacts/reports/synthetic_longitudinal_v3_summary.md",
        help="Summary report markdown path for the v3 cohort",
    )
    parser.add_argument(
        "--policy-report-json",
        default="artifacts/reports/policy_training_v1_from_v3_summary.json",
        help="Derived policy summary report JSON path",
    )
    parser.add_argument(
        "--policy-report-md",
        default="artifacts/reports/policy_training_v1_from_v3_summary.md",
        help="Derived policy summary report markdown path",
    )
    parser.add_argument(
        "--sample-trace-output",
        default="artifacts/reports/synthetic_longitudinal_v3_trace_sample.json",
        help="JSON output path for a short v3 sample trajectory trace",
    )
    return parser
...
def _render_cohort_markdown(summary: dict[str, object]) -> str:
    lines = [
        "# synthetic longitudinal v3 summary",
        "",
...
def _render_policy_markdown(summary: dict[str, object]) -> str:
    lines = [
        "# derived policy training from v3 summary",
        "",
...
def main() -> int:
    ...
    records = generate_rich_synthetic_cohort_v3(seed=args.seed, user_count=args.user_count)
    ...
    cohort_summary = summarize_rich_synthetic_cohort_v3(records, seed=args.seed)
    ...
    write_rich_synthetic_jsonl_v3(cohort_output, records)
