...
def main() -> int:
    ...
    form = form.replace("input[type=text],textarea", "input[type=text],input[type=password],textarea")
    form = form.replace(
        '<p><strong>판정 10건과 판정 이유는 모두 작성되어 있습니다.</strong> 외부 약사는 아래 실제 신원 3개만 입력하고, 판정이 다를 때만 수정하세요.</p><label class="field" for="name">외부 약사 성명</label><input id="name" type="text" autocomplete="name"><label class="field" for="org">실제 소속</label><input id="org" type="text" autocomplete="organization"><label class="field" for="license">약사 면허 식별정보</label><input id="license" type="text">',
        '<p><strong>판정 10건과 판정 이유는 모두 작성되어 있습니다.</strong> 성명과 소속도 미리 채웠습니다. 외부 약사는 최초 한 번만 실제 면허번호를 입력하고, 판정이 다를 때만 수정하세요.</p><label class="field" for="name">외부 약사 성명</label><input id="name" type="text" value="여형준" autocomplete="name"><label class="field" for="org">실제 소속</label><input id="org" type="text" value="연세대학교 약학대학" autocomplete="organization"><label class="field" for="license">약사 면허번호(원문을 저장하지 않음)</label><input id="license" type="password" autocomplete="off"><p>면허번호는 이 브라우저에서 SHA-256 해시로 바뀌며 원문은 저장하거나 전송하지 않습니다.</p>',
    )
    form = form.replace(
        "const profile=JSON.parse(localStorage.getItem('op039ExternalReviewer')||'{}');document.getElementById('name').value=profile.name||'';document.getElementById('org').value=profile.organization||'';document.getElementById('license').value=profile.pharmacist_license_id||'';function buildResult(){const name=document.getElementById('name').value.trim();const organization=document.getElementById('org').value.trim();const pharmacistLicenseId=document.getElementById('license').value.trim();",
        "const profile=JSON.parse(localStorage.getItem('op039ExternalReviewer')||'{}');document.getElementById('name').value=profile.name||'여형준';document.getElementById('org').value=profile.organization||'연세대학교 약학대학';async function sha256(value){const digest=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(value));return Array.from(new Uint8Array(digest)).map(byte=>byte.toString(16).padStart(2,'0')).join('')}async function buildResult(){const name=document.getElementById('name').value.trim();const organization=document.getElementById('org').value.trim();const rawLicense=document.getElementById('license').value.trim();",
    )
    form = form.replace(
        "if(!name||!organization||!pharmacistLicenseId||decisions.some",
        "if(!name||!organization||!rawLicense||decisions.some",
    ).replace(
        "const reviewer={name,organization,pharmacist_license_id:pharmacistLicenseId,",
