...
function operationControls(){const coverage=state.operational_coverage;return `<p><strong>이 컴퓨터에서 실제 연구 기능을 사용하면 영수증은 자동으로 쌓입니다.</strong></p><p>${coverage.covered_count}/${coverage.required_count}개 확정 · 이번 실행에서 ${coverage.current_session_provisional_count}개 감지 · 유효 영수증 ${coverage.valid_receipt_count}개</p><progress value="${coverage.covered_count}" max="${coverage.required_count}" style="width:100%"></progress><div class="row"><a href="${state.operational_urls.user_session}" target="_blank"><button>설문·추천·후속평가 화면 열기</button></a><a href="${state.operational_urls.pharmacist_review}" target="_blank"><button class="secondary">약사 검토 화면 열기</button></a></div><p class="help">이번 실행에서 감지된 수는 아직 확정 증거가 아닙니다. 서버를 종료하면 서명 영수증이 생기고 다음 실행에서 확정 수에 반영됩니다. 서버를 켜기만 한 세션은 세지 않습니다.</p><button onclick="load()">현재 사용 내용 다시 확인</button><button class="secondary" onclick="act('operations_collect',{operator_id:defaultActor})">확정 영수증 다시 확인</button><details><summary>아직 남은 OP 보기</summary><p>${coverage.missing_requirement_ids.join(', ')||'없음'}</p></details>`}
