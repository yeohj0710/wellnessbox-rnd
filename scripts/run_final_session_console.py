...
function controls(id){
 ...
 if(id==='H-006')return `<p class="help">기존 Ed25519 키를 선택하거나 새 키를 만든 뒤 영수증 두 종류를 서명합니다.</p><div class="grid"><input id="keyPath" value="${esc(generatedKeyPath)}" placeholder="Ed25519 키 파일 경로"><input id="issuer" placeholder="발급자 ID"></div><div class="row"><button class="secondary" onclick="generateKey()">이 경로에 키 생성</button><button onclick="signReceipts()">영수증 2종 서명</button></div>`;
...
function operationControls(){const environment=Object.entries(operationLabels).map(([key,label])=>`<div class="evidence-row"><label><input type="checkbox" id="check-${key}"> ${label}</label><input id="evidence-${key}" placeholder="확인 증거 파일 경로"></div>`).join('');const requirements=state.stage_gap_ids.map(id=>`<div class="evidence-row"><label><input type="checkbox" id="check-${id}"> ${id}</label><input id="evidence-${id}" placeholder="${id} PASS 증거 JSON 경로"></div>`).join('');return `<p class="help">체크한 항목의 증거 파일만 검증합니다. 단계 미달 요구는 각 요구 ID와 PASS 상태가 들어 있는 JSON이 필요합니다.</p><input id="operator" placeholder="운영 확인자 ID"><h3>운영 환경</h3>${environment}<details><summary>단계 미달 요구 ${state.stage_gap_ids.length}개 증거 입력</summary><div class="evidence-list">${requirements}</div></details><button onclick="saveOperations()">운영 증거 검증하고 저장</button>`}
function saveOperations(){const checks={};for(const key of Object.keys(operationLabels)){checks[key]={status:$(`check-${key}`).checked?'PASS':'PENDING',evidence:$(`evidence-${key}`).value}}checks.requirement_evidence={};for(const id of state.stage_gap_ids)if($(`check-${id}`).checked)checks.requirement_evidence[id]=$(`evidence-${id}`).value;act('operations',{operator_id:$('operator').value,checks})}
