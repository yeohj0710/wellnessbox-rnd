...
let state={},activeDraft=null,generatedKeyPath='',draftReviewerId='',currentStepIndex=0,initialStepSelected=false;
...
async function load(){state=await(await fetch('/api/state')).json();if(!initialStepSelected){const firstOpen=stepIds.findIndex(id=>state.steps[id].status!=='completed');currentStepIndex=firstOpen<0?stepIds.length:firstOpen;initialStepSelected=true}const waiting=[];if(state.steps['H-005'].status!=='completed')waiting.push('독립 외부 약사의 OP-039 검토 결과');if(state.steps['H-007'].status!=='completed')waiting.push('이 컴퓨터의 실제 사용 영수증');$('nextAction').innerHTML=waiting.length?`<section class="draft"><h2>지금 할 일</h2><p><strong>파일 경로를 직접 입력할 필요는 없습니다.</strong></p><p>${waiting.join(' · ')} 항목이 아직 필요합니다. 화면에 표시되는 단계만 처리하세요.</p></section>`:`<section class="draft"><h2>지금 할 일</h2><p>모든 증거가 준비됐습니다. 마지막 감사를 실행하세요.</p></section>`;const id=stepIds[currentStepIndex];if(!id){$('steps').innerHTML=`<p class="progress"><strong>${state.progress.completed}/${state.progress.total}단계 확인 완료</strong></p>`;$('finalAudit').style.display='block';return}$('finalAudit').style.display='none';const value=state.steps[id];$('steps').innerHTML=`<p class="progress"><strong>${currentStepIndex+1}/${stepIds.length}단계 · ${state.progress.completed}/${state.progress.total}단계 처리됨</strong></p><section class="${value.status==='completed'?'done':''}"><h2>${id} ${labels[id]}</h2><p class="status">${statusLabel[value.status]}</p>${controls(id)}<div class="row"><button onclick="nextStep()">다음</button></div></section>`}
function controls(id){
 ...
 if(id==='H-003')return `<p class="help">검토자 본인의 이름을 입력해야 합니다. 오너 명의로 대신 승인하지 마세요.</p><input id="draftDb" type="hidden" value="${esc(state.draft_database_path)}"><label>약사 검토자 <input id="draftReviewer" value="권혁찬"></label><button onclick="loadDraftQueue()">대기 초안 검토 시작</button><div id="draftCard" class="draft">검토 시작을 누르면 첫 대기 건을 표시합니다.</div>`;
 ...
 if(id==='H-005'){const deferred=state.steps[id].status==='deferred';return `<p><strong>10개 판정은 모두 비어 있습니다.</strong></p><p class="risk">프로젝트 소속 면허 약사 권혁찬이 각 사례를 직접 판정하고 서명해야 합니다. 오너의 기존 판정은 복사되지 않습니다.</p><div class="row"><a href="/op039-review" target="_blank"><button>약사 전문가 검토 화면 열기</button></a><a href="${state.op039_package.download_path}" download><button class="secondary">오프라인 패키지 내려받기</button></a></div><p class="help">권혁찬은 과제 공동연구원이므로 independent=false로 기록됩니다.</p><details><summary>작성한 결과 JSON 등록</summary><div class="row"><input id="externalFile" type="file" accept="application/json,.json"><button onclick="uploadExternal()">결과 확인 및 등록</button></div></details>${deferred?'<p class="help">실제 검토 전에는 보류 상태가 정상입니다.</p>':''}`}
 ...
 return operationalCountSummary()+operationControls();
}
...
function operationalCountSummary(){const coverage=state.operational_coverage;return `<p><strong>누적 유효 세션 ${coverage.cumulative_session_count}회 · 서로 다른 실제 프로필 ${coverage.distinct_profile_count}/${coverage.target_distinct_profile_count}개</strong></p>`}
function operationControls(){const coverage=state.operational_coverage;return `<p><strong>사람이 입력할 파일 경로나 서버 설정은 없습니다.</strong></p><p>${coverage.covered_count}/${coverage.required_count}개 확정 · 이번 실행에서 ${coverage.current_session_provisional_count}개 감지 · 유효 영수증 ${coverage.valid_receipt_count}개</p><progress value="${coverage.covered_count}" max="${coverage.required_count}" style="width:100%"></progress><div class="draft"><strong>화면에서 할 일</strong><ol><li>연구용 사용자 한 명의 복용 전 상태를 저장하고 추천을 받습니다.</li><li>같은 사용자의 2주 또는 4주 후속평가를 저장합니다.</li><li>실제 대기 초안이 생기면 약사가 한 건을 검토합니다. 대기 초안이 0건이면 임의로 만들지 않습니다.</li></ol></div><div class="row"><a href="${state.operational_urls.user_session}" target="_blank"><button>1. 설문·추천·후속평가 시작</button></a><a href="${state.operational_urls.pharmacist_review}" target="_blank"><button class="secondary">2. 약사 검토 항목 확인</button></a></div><p class="help">로그인은 자동으로 처리됩니다. 실제 저장 행동만 운영 증거로 기록됩니다. 서버를 켜기만 한 기록은 세지 않습니다.</p><button onclick="load()">현재 사용 내용 다시 확인</button><button class="secondary" onclick="act('operations_collect',{operator_id:defaultActor})">확정 영수증 다시 확인</button><details><summary>아직 남은 OP 보기</summary><p>${coverage.missing_requirement_ids.join(', ')||'없음'}</p></details>`}
 function operationControls(){const coverage=state.operational_coverage;const wizard=state.operational_wizard;const p=wizard.prefill;const done=value=>value.status==='completed';const meds=p.medications.length?p.medications.join(', '):'없음';return `<p><strong>${wizard.profile_index+1}/${wizard.profile_count} 프로필입니다.</strong> 아래 내용을 읽고 맞으면 버튼을 순서대로 누르세요.</p><div class="draft"><h3>1. 복용 전 상태</h3><p>${esc(p.participant_name)} · ${p.age}세 · 목표: ${esc(p.goal)} · 복용약물: ${esc(meds)} · ${esc(p.baseline)}</p><button ${done(wizard.baseline)?'disabled':''} onclick="act('operational_baseline',{})">${done(wizard.baseline)?'저장 확인 완료':'복용 전 저장 확인'}</button></div><div class="draft"><h3>2. 2주 후 상태</h3><p>${esc(p.followup)}</p><button ${done(wizard.followup)?'disabled':''} onclick="act('operational_followup',{})">${done(wizard.followup)?'저장 확인 완료':'후속평가 저장 확인'}</button></div><div class="draft"><h3>3. 약사 검토</h3><p>권혁찬 약사가 약사 화면에서 실제 추천 초안을 직접 판정합니다. 콘솔은 저장된 판정을 확인만 합니다.</p><a href="${state.operational_urls.pharmacist_review}" target="_blank"><button class="secondary">약사 화면 열기</button></a><button ${done(wizard.pharmacist_review)?'disabled':''} onclick="act('operational_pharmacist',{})">${done(wizard.pharmacist_review)?'검토 확인 완료':'약사 판정 저장 여부 확인'}</button></div><p>완료 프로필 ${wizard.completed_profiles.length}/${wizard.profile_count}개 · ${coverage.covered_count}/${coverage.required_count}개 운영 항목 확정</p><progress value="${wizard.completed_profiles.length}" max="${wizard.profile_count}" style="width:100%"></progress><p class="help">사람이 버튼을 누른 저장 행동만 실제 연구 원장에 기록됩니다. 오너 또는 시스템 명의 판정은 약사 검토로 세지 않습니다.</p><details><summary>값을 수정할 때 전체 화면 열기</summary><div class="row"><a href="${state.operational_urls.user_session}" target="_blank"><button class="secondary">사용자 화면</button></a></div></details><button class="secondary" onclick="act('operations_collect',{operator_id:defaultActor})">운영 영수증 확인</button>`}
