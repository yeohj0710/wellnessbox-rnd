...
function controls(id){
 ...
 if(id==='H-005')return `<p class="help">OP-039 외부 평가 결과 JSON을 선택하면 콘솔이 저장 위치와 검증을 처리합니다.</p><div class="row"><input id="externalFile" type="file" accept="application/json,.json"><button onclick="uploadExternal()">결과 확인 및 등록</button><button class="secondary" onclick="act('external_upload',{document:null})">나중에 등록</button></div>`;
...
function operationControls(){const saved=state.steps['H-007'];const registered=saved.registered_requirement_evidence||{};return `<p class="help">실제 배포·운영 증거 JSON을 한꺼번에 선택하세요. 콘솔이 항목을 구분하고 저장·검증합니다.</p><p>${Object.keys(registered).length}/${state.stage_gap_ids.length}개 요구 증거 등록됨</p><input id="operationFiles" type="file" accept="application/json,.json" multiple><div class="row"><button onclick="uploadOperations()">운영 증거 자동 분류 및 확인</button><button class="secondary" onclick="act('operations_upload',{operator_id:defaultActor,documents:[]})">준비된 증거 없음</button></div>`}
