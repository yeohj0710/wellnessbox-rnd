...
let state={},activeDraft=null,generatedKeyPath='',draftReviewerId='',currentStepIndex=0,initialStepSelected=false,pharmacistReviewOpened=sessionStorage.getItem('operationalPharmacistReviewOpened')==='true';
...
async function act(action,payload){const primary=$('primaryAction');primary.disabled=true;primary.textContent='저장하고 있습니다…';try{await api(action,payload);if(action==='operational_pharmacist'){sessionStorage.removeItem('operationalPharmacistReviewOpened');pharmacistReviewOpened=false}await load();return true}catch(error){$('result').textContent=error.message;console.error(error);syncActionDock();return false}}
