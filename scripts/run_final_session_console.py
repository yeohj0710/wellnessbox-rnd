...
@media(max-width:640px){body{padding:36px 16px 162px}.shell{padding:24px 18px;border-radius:22px}h1{font-size:30px}.intro{font-size:16px;margin-bottom:26px}.grid{grid-template-columns:1fr}.evidence-row{grid-template-columns:1fr}input,select{width:100%}.current-card{padding:20px 16px}.action-dock{padding:12px 16px calc(12px + env(safe-area-inset-bottom))}.action-inner{grid-template-columns:1fr;gap:8px}.action-label{display:none}.action-title{font-size:14px}.primary-action{min-height:52px}.step-strip{gap:5px}}
 ...
 let state={},activeDraft=null,draftReviewerId='',currentStepIndex=0,initialStepSelected=false,lastActionError='',pharmacistReviewOpened=sessionStorage.getItem('operationalPharmacistReviewOpened')==='true';
...
async function act(action,payload){const primary=$('primaryAction');lastActionError='';primary.disabled=true;primary.textContent='저장하고 있습니다…';try{await api(action,payload);if(action==='operational_pharmacist'){sessionStorage.removeItem('operationalPharmacistReviewOpened');pharmacistReviewOpened=false}await load();return true}catch(error){lastActionError=error.message;$('result').textContent=error.message;console.error(error);syncActionDock();return false}}
...
async function load(){state=await(await fetch('/api/state')).json();const firstOpen=stepIds.findIndex(id=>!stepCompleted(id));if(!initialStepSelected||currentStepIndex>=stepIds.length||stepCompleted(stepIds[currentStepIndex])){currentStepIndex=firstOpen<0?stepIds.length:firstOpen;initialStepSelected=true}renderStepStrip();const id=stepIds[currentStepIndex];const completedCount=stepIds.filter(stepCompleted).length;if(!id){$('nextAction').innerHTML='<p class="metric">7개 확인 단계가 모두 끝났습니다.</p>';$('steps').innerHTML='';$('finalAudit').style.display='block';syncActionDock();return}$('finalAudit').style.display='none';const displayStatus=stepCompleted(id)?'completed':id==='H-007'?'deferred':state.steps[id].status;$('nextAction').innerHTML=`<p class="metric">${currentStepIndex+1}/7단계 · ${completedCount}/7단계 처리됨</p>`;$('steps').innerHTML=`<section class="current-card ${displayStatus==='completed'?'done':''}"><p class="eyebrow">${id} · ${statusLabel[displayStatus]}</p><h2>${labels[id]}</h2>${controls(id)}</section>`;syncActionDock()}
 function dockAction(){const id=stepIds[currentStepIndex];if(!id)return{title:'모든 확인 단계가 끝났습니다.',label:'최종 감사 실행',run:()=>act('audit',{})};if(id==='H-007'){const wizard=state.operational_wizard;if(wizard.baseline.status!=='completed')return{title:`프로필 ${wizard.profile_index+1}: 복용 전 상태를 저장합니다.`,label:'복용 전 상태 저장',run:()=>act('operational_baseline',{})};if(wizard.followup.status!=='completed')return{title:`프로필 ${wizard.profile_index+1}: 2주 후 상태를 저장합니다.`,label:'후속평가 저장',run:()=>act('operational_followup',{})};if(wizard.pharmacist_review.status!=='completed'&&!pharmacistReviewOpened)return{title:'예비 약사 사전 검토자가 추천 초안을 확인합니다.',label:'예비 약사 사전 검토 열기',run:()=>{sessionStorage.setItem('operationalPharmacistReviewOpened','true');location.href=state.operational_urls.pharmacist_review}};if(wizard.pharmacist_review.status!=='completed')return{title:'예비 약사 사전 검토를 저장한 뒤 이 버튼을 누르세요.',label:'사전 검토 완료 확인',run:()=>act('operational_pharmacist',{})};return{title:'이 프로필의 세 단계가 끝났습니다.',label:'다음 프로필로 이동',run:async()=>{await load()}}}if(id==='H-001')return{title:'연구계획 정렬 결과를 확인합니다.',label:'정렬 결과 확인',run:()=>act('alignment',{reviewer_id:defaultActor})};if(id==='H-002')return{title:'안전 정책 9개를 한 번에 확인합니다.',label:'정책 전체 확인',run:()=>act('policy_all',{reviewer_id:defaultActor})};if(id==='H-003')return{title:'예비 약사 사전 검토 대기 초안을 불러옵니다.',label:'대기 초안 보기',run:()=>loadDraftQueue()};if(id==='H-004')return{title:'보고서 문체 표본을 승인합니다.',label:'문체 확인',run:()=>tone(true)};if(id==='H-005')return{title:'OP-039 예비 약사 사전 검토 화면을 엽니다.',label:'예비 약사 사전 검토 열기',run:()=>{location.href='/op039-review'}};return{title:'두 발급자가 제공한 서명 키로 영수증을 등록합니다.',label:'영수증 등록',run:()=>prepareAndSignReceipts()}}
function syncActionDock(){const action=dockAction();const title=$('dockTitle');title.textContent=lastActionError?'저장하지 못했습니다. 같은 버튼을 다시 눌러주세요.':action.title;title.style.color=lastActionError?'var(--danger)':'';const button=$('primaryAction');button.textContent=lastActionError?'다시 시도':action.label;button.disabled=false}
...
function controls(id){
 ...
 if(id==='H-003')return `<p class="help">검토자 본인의 이름을 확인하고 화면 아래의 고정 버튼으로 대기 초안을 불러오세요.</p><input id="draftDb" type="hidden" value="${esc(state.draft_database_path)}"><label>초안 검토자 <input id="draftReviewer" value="권혁찬"></label><div id="draftCard" class="draft">대기 초안을 불러오면 여기에 표시됩니다.</div>`;
 ...
 if(id==='H-005'){const deferred=state.steps[id].status==='deferred';return `<p class="risk">예비 약사 사전 검토자가 10개 사례를 직접 판정하고 서명해야 합니다.</p><p class="help">오너의 기존 판정은 복사하지 않으며, 검토자의 신원·소속·자격·서명을 기록합니다.</p><details class="advanced"><summary>오프라인 검토가 필요할 때</summary><p><a href="${state.op039_package.download_path}" download>오프라인 패키지 내려받기</a></p><input id="externalFile" type="file" accept="application/json,.json"><button class="secondary" onclick="uploadExternal()">작성한 JSON 등록</button></details>${deferred?'<p class="help">실제 검토 전에는 보류 상태가 정상입니다.</p>':''}`}
 if(id==='H-006'){const saved=state.steps['H-006'];return `<p class="help">검증 담당자와 독립 검토자가 제공한 서로 다른 기존 Ed25519 키를 입력하세요. 키를 자동 생성하거나 영수증을 대신 서명하지 않습니다.</p><div class="grid"><label>검증용 키 경로<input id="validationKeyPath" type="text" value="${esc(saved.validation_key_path||'')}" autocomplete="off"></label><label>검증 발급자 ID<input id="validationIssuer" type="text" value="${esc(saved.validation_issuer_id||'')}" autocomplete="off"></label><label>독립 검토용 키 경로<input id="independentReviewKeyPath" type="text" value="${esc(saved.independent_review_key_path||'')}" autocomplete="off"></label><label>독립 검토 발급자 ID<input id="independentReviewIssuer" type="text" value="${esc(saved.independent_review_issuer_id||'')}" autocomplete="off"></label></div>`}
