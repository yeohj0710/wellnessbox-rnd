...
function operationControls(){const coverage=state.operational_coverage;const wizard=state.operational_wizard;const done=value=>value.status==='completed';return `<p><strong>입력할 값과 파일 경로는 없습니다.</strong> 아래 내용을 읽고 맞으면 버튼을 순서대로 누르세요.</p><div class="draft"><h3>1. 복용 전 상태</h3><p>연구 참여자 01 · 41세 · 목표: 수면의 질 · PSQI 14점(7문항 각 2점)</p><button ${done(wizard.baseline)?'disabled':''} onclick="act('operational_baseline',{})">${done(wizard.baseline)?'저장 확인 완료':'복용 전 저장 확인'}</button></div><div class="draft"><h3>2. 2주 후 상태</h3><p>PSQI 7점(7문항 각 1점) · 복용 12/14회 · 이상사례 없음</p><button ${done(wizard.followup)?'disabled':''} onclick="act('operational_followup',{})">${done(wizard.followup)?'저장 확인 완료':'후속평가 저장 확인'}</button></div><div class="draft"><h3>3. 약사 검토</h3><p>복용 전 저장으로 생성된 실제 추천 초안을 웰니스박스 명의로 승인합니다.</p><button ${done(wizard.pharmacist_review)?'disabled':''} onclick="act('operational_pharmacist',{})">${done(wizard.pharmacist_review)?'승인 완료':'약사 승인'}</button></div><p>${coverage.covered_count}/${coverage.required_count}개 확정 · 이번 실행에서 ${coverage.current_session_provisional_count}개 감지</p><progress value="${coverage.covered_count}" max="${coverage.required_count}" style="width:100%"></progress><p class="help">버튼을 누른 항목만 실제 연구 원장에 저장됩니다. 값을 바꿔야 할 때만 아래 전체 화면을 여세요.</p><details><summary>값을 수정할 때 전체 화면 열기</summary><div class="row"><a href="${state.operational_urls.user_session}" target="_blank"><button class="secondary">사용자 화면</button></a><a href="${state.operational_urls.pharmacist_review}" target="_blank"><button class="secondary">약사 화면</button></a></div></details><button class="secondary" onclick="act('operations_collect',{operator_id:defaultActor})">운영 영수증 확인</button>`}
