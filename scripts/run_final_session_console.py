...
function syncActionDock(){const action=dockAction();$('dockTitle').textContent=action.title;const button=$('primaryAction');button.textContent=action.label;button.disabled=false;button.onclick=action.run}
async function runPrimaryAction(){return dockAction().run()}
