...
def main() -> int:
    ...
    checks = {
        ...
        "secret_is_server_owned": "WB_RND_SERVICE_TOKEN" in blobs[SERVICE_PATHS[0]] and "NEXT_PUBLIC_WB_RND" not in combined,
        "url_and_timeout_fail_closed": "contains_credentials_or_suffix" in combined and "out_of_range" in combined,
        "remote_execution_origin_is_explicit": 'kind: "rnd_execution"' in combined and "R&D 실행 결과" in combined,
        "local_snapshot_origin_is_explicit": 'kind: "local_snapshot"' in combined and "로컬 스냅샷 결과" in combined,
        ...
        "focused_contract_test_is_committed": "environment and result-origin contract: PASS" in combined,
    }
    ...
    report = {
        ...
        "dataset": {"path": DATASET.relative_to(ROOT).as_posix(), "sha256": hashlib.sha256(dataset_bytes).hexdigest(), "case_count": len(dataset["cases"])},
        "checks": checks,
        "source_identity": {"wellnessbox_commit": service_commit, "wellnessbox_source_sha256": hashlib.sha256(combined.encode()).hexdigest()},
        "stage_boundary": {"local_service_integration_proven": True, "vercel_environment_registered": False, "production_traffic_proven": False},
    }
