...
def build_parser() -> ArgumentParser:
    parser = ArgumentParser(
        description=(
            "Build a bounded replay-only diagnostic for the mid_margin bucket inside "
            "the threshold_duration_sensitive subgroup."
        )
    )
    parser.add_argument(
        "--dataset",
    ...
    parser.add_argument(
        ...
        help="Current subgroup narrowing decision JSON path.",
    )
    parser.add_argument(
        "--max-cycles",
        type=int,
    ...
    parser.add_argument(
        ...
        default=(
            "artifacts/reports/"
            "non_cgm_threshold_duration_sensitive_mid_margin_diagnostic_v1.md"
        ),
