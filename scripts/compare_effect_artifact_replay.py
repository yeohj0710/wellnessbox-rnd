...
def build_parser() -> ArgumentParser:
    parser = ArgumentParser(
        description="Compare two replay-only effect artifacts on the fixed v4 reshaped cohort"
    )
    parser.add_argument(
        "--dataset",
    ...
    parser.add_argument(
        ...
        default="artifacts/models/effect_model_v3_retrained_candidate.json",
    ...
    parser.add_argument(
        "--candidate-label",
        default="retrained_effect",
        help="Candidate artifact label",
    )
    parser.add_argument(
        "--report-json",
        default="artifacts/reports/effect_artifact_replay_comparison_v1.json",
        help="Comparison report JSON output path",
    )
    parser.add_argument(
        "--report-md",
        default="artifacts/reports/effect_artifact_replay_comparison_v1.md",
...
def main() -> int:
    ...
    candidate_report = _build_artifact_report(
        ...
        effect_artifact_path=args.candidate_effect_artifact,
        policy_artifact_path=args.policy_artifact,
        dataset_path=args.dataset,
        split=split,
    )
    ...
    *,
    ...
    return {
        "test_aggregate_mae_delta": round(
            candidate["test_metrics"]["aggregate_mae"]
            - reference["test_metrics"]["aggregate_mae"],
            6,
        ),
        "test_aggregate_r2_delta": round(
            candidate["test_metrics"]["aggregate_r2"]
            - reference["test_metrics"]["aggregate_r2"],
            6,
        ),
