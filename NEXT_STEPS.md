...
## 2026-07-27 사람 최종 세션 전 다음 세 반복

1. H-005 입력을 중립화한다. 10건의 판정과 의견 선입력을 없애고, 검토자 신원을 오너 원장과 대조하며, 실제 면허 ID·자격 확인 방법·별도 서명을 검증한다. H-003 검토자 대조와 신뢰 원장 대체 경로에도 같은 자격 검증을 적용한다.
2. H-003 승인 초안 계보를 구현한다. 승인·수정 승인 초안 ID만 담은 데이터셋 manifest, 후보 artifact 학습 명령, 후보 모델을 받는 256건 고정 평가 실행기, 안전 회귀 실패 조건, 교체·유지와 rollback 영수증이 필요하다. 학습 게이트가 열리기 전에는 훈련하지 않는다.
3. 앞의 두 반복과 실제 후속 자료 준비가 끝나면 `docs/original_plan/FINAL_SESSION_RUNBOOK.md`로 통제된 사람 세션을 수행한다. 먼저 무영수증 preflight가 `READY`인지 보고, 서로 다른 실제 프로필 5건과 사람 판정·서명을 요구사항별 관찰값에 연결한다.

현재 OP-120 감사는 R&D 입력 커밋 `821a5a5`로 120/120 `READY`다. 등록 파일 경로 492건은 모두 존재하고 보고서의 남은 내용 불일치는 0건이다. 다만 감사 `READY`와 DB의 프로필 5/5는 유효한 사람 세션 5건, 중립적 사람 검토와 자격 확인, 승인 초안 학습을 입증하지 않는다. Ruff 32건은 main 조상 커밋에서 들어온 기존 정적 검사 부채다.
