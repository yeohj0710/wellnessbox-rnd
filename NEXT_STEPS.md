...
## 2026-07-30 예비 약사 자격 구조 반영 이후 다음 세 반복

1. 학습 게이트를 여는 CGM 최종 단계 기하 blocker를 해소한다. H-003 명령 체인은 모두 구현했지만 `build_training_readiness_gate.py`가 여전히 `no_go_keep_training_blocked`라 후보 모델을 만들 수 없다.
2. 서로 다른 실제 프로필 5건의 전체 경로를 사람이 실행한다. 화면과 백엔드가 모두 준비됐고 사전 점검이 `READY`이므로, 남은 것은 실제 참여자 자료와 동의 근거로 H-007을 채우는 일이다.
3. 2차년도 최종 세션을 예비 약사 검토로 끝까지 수행한다. `docs/original_plan/FINAL_SESSION_RUNBOOK.md` 순서대로 H-002~H-006을 진행한다. 2차년도에는 연구를 완료할 의무가 없으므로 예비 약사 사전 검토까지가 이번 범위다.

3차년도(2027-01 면허 취득 이후)에 할 일은 별도다. `op039_reviewer_identity_registry_v1.json`의 `qualification_stage`를 `licensed_pharmacist`로 바꾸고 같은 10개 사례를 약사 자격으로 다시 검토한다. 그때부터 "약사가 검토·평가했다"고 쓸 수 있다. 최종 마감은 2027-10이다. OP-039 외부 기관 평가는 연구 종료 후 외부 기관이 수행하는 또 다른 단계이며, 두 신뢰 원장이 0건이라 현재 그 경로는 닫혀 있다.

현재 사전 점검은 `READY`, 종료 코드 0, 차단 0건이다. OP-120 감사는 120/120 `READY`다. 다만 이 두 신호는 다섯 건의 유효한 사람 세션도, 약사 자격 검토도 입증하지 않는다. Ruff 28건은 main 조상 커밋에서 들어온 기존 정적 검사 부채다.
