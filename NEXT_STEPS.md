...
1. H-005 백엔드 자격 검증을 강화한다. 화면은 중립화했지만 오너 차단이 아직 `여형준`·`웰니스박스` 문자열 비교라 별칭을 막지 못한다. 인증 계정·오너 원장과 연결하고, `was_ai_draft_reviewer`를 H-003 검토자 원장과 대조하며, 면허 ID의 형식과 실재를 확인하고, 신뢰 원장 대체 경로에도 같은 자격 검증을 건다.
2. 학습 게이트를 연다. H-003 명령 체인은 모두 구현했지만 `build_training_readiness_gate.py`가 여전히 `no_go_keep_training_blocked`다. CGM 최종 단계 기하 blocker를 해소해야 후보 모델 학습과 후보 평가, 교체 판정을 실제 값으로 돌릴 수 있다.
3. 통제된 사람 최종 세션을 수행한다. 실제 후속평가 자료 5건이 준비됐고 서명 키는 기존 것을 쓰기로 정했으므로, `docs/original_plan/FINAL_SESSION_RUNBOOK.md` 순서대로 진행한다. 사전 점검이 `READY`인지 먼저 확인하고, 요구사항별 관찰값에 사람 판정과 서명을 연결한다.

현재 사전 점검은 `READY`, 종료 코드 0, 차단 0건이다. OP-120 감사는 R&D 입력 커밋 `4b5b9e9`로 120/120 `READY`다. 다만 이 두 신호는 다섯 건의 유효한 사람 세션이나 독립 외부 검토를 입증하지 않는다. OP-039 외부 기관 섭외는 별도 진행 중이며 그전까지 manifest의 `EXTERNAL` 표시는 미충족 조건에 걸려 있다. Ruff 29건은 main 조상 커밋에서 들어온 기존 정적 검사 부채다.
