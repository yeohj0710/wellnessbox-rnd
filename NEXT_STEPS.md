...
## 2026-07-27 H-005·H-003 수정 이후 다음 세 반복

1. 학습 게이트를 여는 CGM 최종 단계 기하 blocker를 해소한다. H-003 명령 체인은 모두 구현했지만 `build_training_readiness_gate.py`가 여전히 `no_go_keep_training_blocked`라 후보 모델을 만들 수 없다. 게이트가 열려야 후보 학습, 후보 평가, 교체 판정을 실제 값으로 돌릴 수 있다.
2. 서로 다른 실제 프로필 5건의 전체 경로를 사람이 실행한다. 화면과 백엔드 모두 준비됐고 사전 점검이 `READY`이므로, 남은 것은 실제 참여자 자료와 동의 근거로 H-007을 채우는 일이다.
3. 통제된 사람 최종 세션을 끝까지 수행한다. 서명 키는 기존 것을 쓰기로 정했으므로 `docs/original_plan/FINAL_SESSION_RUNBOOK.md` 순서대로 H-002~H-006을 진행하고, 요구사항별 관찰값에 사람 판정과 서명을 연결한다. 연구 종료 후 외부 기관 평가는 그다음 단계다.

현재 사전 점검은 `READY`, 종료 코드 0, 차단 0건이다. OP-120 감사는 R&D 입력 커밋 `4b5b9e9`로 120/120 `READY`다. 다만 이 두 신호는 다섯 건의 유효한 사람 세션을 입증하지 않는다. OP-039 외부 기관 평가는 연구가 끝난 뒤에 외부 기관이 수행하는 별개 단계이므로 이번 연구 기간의 과제가 아니다. 두 신뢰 원장이 아직 0건이라 그 경로는 닫혀 있고, manifest의 `EXTERNAL` 표시도 미충족 조건에 걸린 상태로 두는 것이 맞다. Ruff 29건은 main 조상 커밋에서 들어온 기존 정적 검사 부채다.
