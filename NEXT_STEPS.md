...
1. Implement OP-010: generate a Korean completion report listing complete, partial, pending, external, and contradicted requirements directly from the validated manifest.
2. Start OP-011 and OP-012: complete the biometric profile, structured symptom severity, and urgent-risk input contracts without duplicating the existing safety fields.
3. Implement OP-013 and OP-014: complete medication and current-supplement dose/unit contracts before building the cross-repository profile adapter.

After OP-010, continue OP-011 through OP-020 in bounded slices and add the lossless `wellnessbox` profile-to-R&D adapter.
