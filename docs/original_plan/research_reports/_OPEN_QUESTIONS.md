...
신뢰 원장 경로를 "약사 자격 없이 완료되는 우회 경로"로 적었던 이전 기록은 잘못이다. 그 경로는 프로젝트 소속 약사 검토를 대신하는 우회로가 아니라, 연구가 끝난 뒤 외부 기관이 수행하는 평가를 받는 별개 입력이다. 성격이 다르므로 약사 면허를 요구하지 않는 것이 맞다. 게다가 두 신뢰 원장(`op039_external_coverage_trust_roots_v1.json`, `op039_external_attestation_trust_roots_v1.json`)이 모두 0건이라 현재 이 경로는 아예 발동하지 않는다.
...
사람 판정 데이터는 이번에도 만들지 않았다.

#### 2026-07-27 저녁에 해소한 백엔드 검증

화면에 이어 백엔드 자격 검증 네 구간도 채웠다.

- 오너 차단이 이름 두 개 문자열 비교였다. 이제 NFKC 정규화 뒤 공백과 제로폭 문자를 지우고 대조하며, 차단 대상과 별칭은 `data/original_plan/contracts/op039_reviewer_identity_registry_v1.json`에 둔다. `여 형준`, `여​형‌준`, `Yeo Hyeongjun`, `hjyeo`, `wellness box` 같은 변형이 막힌다.
- 면허 ID가 빈 문자열만 거부했다. 이제 `not_collected`·`미수집`·`n/a`·`-` 같은 자리표시자를 거부하고 숫자 4자리 이상을 요구한다.
- 자격 확인 방법을 아예 읽지 않았다. 이제 `project_owner_attestation`과 자기 확인류를 거부하고 8자 이상의 실제 설명을 요구한다.
- `was_ai_draft_reviewer`가 자기 신고였다. 이제 실제 초안 원장을 읽기 전용으로 열어 대조하며, 원장에 이름이 있는데 false로 신고하면 차단한다. 실제 원장의 H-003 검토자는 권혁찬과 웰니스박스다.
- 전용 시험 28건이 통과한다(`tests/test_reviewer_credentials.py`). 별칭·제로폭·공백 우회, 자리표시자 면허, 오너 확인 방법, 원장 불일치를 각각 막는지 확인한다.

남은 한계: 면허 번호의 형식만 보고 실제 발급 여부를 조회하지는 않는다. 값의 진위는 여전히 사람이 현장에서 확인해야 한다.
