# 권혁찬 예비 약사·오너 최종 세션 실행서

## 먼저 확인할 결론

이 세션의 실행 순서는 **H-007 실제 5개 프로필 → 생성된 AI 초안 확인 → H-003 권혁찬 예비 약사 전수 검토 → 승인 초안의 학습·평가 반영 확인 → H-002 정책 승인과 H-004 문체 승인 → H-005 고위험 10건 예비 약사 안전 검토 → H-006 최종 영수증**이다.
...
이 문서는 판단 결과, 승인, 서명, 실제 사용 값을 대신 입력하지 않는다. 표에 적은 H-007 프로필은 입력 분산을 확인하기 위한 **제안안**이다. 운영자는 실제 참여자 자료와 대조해 고쳐야 한다. H-005의 `시스템 신호`도 검토자의 결론이 아니다.

## 누가 무엇을 맡는가

| 역할 | 맡을 일 | 대신할 수 없는 결정 |
|---|---|---|
| 운영 보조자 | 명령 실행, 서버 상태 확인, 실제 자료 준비, 화면 전환, 증거 경로 기록 | 검토자 판정, 오너 승인, 서명 |
| 권혁찬 예비 약사 | H-003 전수 검토, H-002 정책 검토, H-005 고위험 10건 판정 | 시스템이나 오너가 대신 입력할 수 없음 |
| 오너 | H-004 보고서 문체 승인, H-006 최종 영수증 발급·확인 | 검토자 판정을 대신할 수 없음 |

권혁찬 예비 약사는 H-003부터 H-005까지 한 번에 처리한다. 운영 보조자는 권혁찬 예비 약사가 도착하기 전에 H-007과 초안 생성 여부를 끝내고, 각 검토 화면을 바로 열 수 있게 준비한다. 오너는 H-004와 H-006 때만 참여한다.
...
이 값은 저장된 과거 상태라는 사실만 보여 준다. 이번 세션에서 권혁찬 예비 약사가 실제 초안을 검토했다는 증거로 재사용하거나, 기존 H-005 판정을 복사하거나, 기존 서명을 새 세션의 서명으로 간주하지 않는다. **과거 상태를 보존한 채 새 세션을 여는 방법은 확인 필요**다. 확인 범위는 다음 파일이다.
...
| 3 | H-003 전수 검토 | 권혁찬 예비 약사 | 대기열과 근거 표시 | 60~120분 | 모든 초안이 승인·수정 승인·반려 중 하나이며 대기 0건 | 빈 대기열을 실제 검토로 확인하려는 경우 |
| 4 | 승인 초안 학습·평가 1회 | 운영 보조자, 결과는 검토자에게 보고 | H-003 대기 0건, 학습 게이트 GO | 30~60분 | 승인 초안만 입력되고 평가 결과·모델 변경 여부가 기록됨 | 학습 게이트가 NO-GO, 안전 회귀 발생 |
| 5 | H-002 정책 승인 | 권혁찬 예비 약사 | H-003과 1회 평가 결과 | 10분 | 9개 규칙을 모두 확인하고 실제 승인 또는 수정 의견 저장 | 불리한 상태를 완화하는 임의 변경 |
| 6 | H-004 보고서 문체 승인 | 오너 | 검토할 실제 보고서 2~3편 | 10~15분 | 표본 2~3편에 실제 승인 또는 의견 저장 | 표본 본문이 열리지 않음 |
| 7 | H-005 고위험 10건 검토 | 권혁찬 예비 약사 | 중립 화면 | 30~45분 | 10건 각각 실제 판정·근거와 실제 서명 저장 | 선택·의견이 미리 채워짐 |
...
모든 입력이 미리 준비됐고 학습 게이트가 열렸을 때 사람과 운영 보조자의 합산 활동 시간은 **약 215~365분(3시간 35분~6시간 5분)**이다. 학습 게이트가 NO-GO면 4단계는 실행 계획만 남기고 멈추므로 그만큼 짧아진다. 권혁찬 예비 약사의 예상 활동 시간은 100~175분, 오너의 예상 활동 시간은 15~25분이다.
...
실행 전후에는 실제 DB 본체·WAL·SHM, 운영 캡처와 프로세스 제어 파일, 최종 세션 루트의 직접 파일, 운영 영수증 전체를 파일별 hash로 비교한다. 사람 입력, 검토자 판정, 승인, 서명, 실제 운영 영수증은 만들지 않는다.
...
12. 검토가 필요한 초안은 판단하지 말고 H-003 대기열에 남긴다. 운영 보조자는 승인·수정 승인·반려를 누르지 않는다.
...
- 복용 전 입력, 추천, 후속평가, 필요한 검토가 같은 프로필·실행·계획 계보로 연결된다.
...
## H-003: 생성된 AI 초안을 권혁찬 예비 약사가 전수 검토한다
...
4. 권혁찬 예비 약사가 직접 `승인`, `수정 승인`, `반려` 중 하나를 누른다.
5. `수정 승인`이면 권혁찬 예비 약사가 수정 내용을 입력한다. `반려`이면 권혁찬 예비 약사가 반려 이유를 입력한다.
6. 화면이 다음 건으로 자동 이동하면 같은 절차를 반복한다.
7. 대기 건수가 0이 될 때까지 계속한다.
8. 생성 수, 검토 수, 승인 수, 수정 승인 수, 반려 수, 대기 수를 기록한다. 승인율이나 처리 속도로 검토 품질을 판정하지 않는다.
...
아래 표는 `data/original_plan/closed_loop_next_action_policy_v1.json`의 우선순위 순서다. 상태는 모두 `FOLLOWUP_ACTIVE`다. 코드는 위에서부터 처음 일치한 규칙 하나를 적용한다. 권혁찬 예비 약사는 표를 읽고 화면에서 실제 승인 또는 수정 의견을 입력한다. 이 표는 승인을 미리 선택하지 않는다.
...
5. 최종 확인 화면에서 `정책 전체 확인` 또는 `규칙 수정 의견 남기기`를 권혁찬 예비 약사가 직접 누른다.
...
## H-005: 권혁찬 예비 약사가 고위험 10건을 빈 선택지에서 판단한다

### 2차년도 검토는 예비 약사 사전 검토다

권혁찬은 2026-07 현재 약사 면허를 받지 않았고 2027-01에 받을 예정이다. 그래서 이번 검토는 **예비 약사 사전 검토**로 기록한다. 면허를 받는 3차년도에 같은 10개 사례를 약사 자격으로 다시 검토해야 "약사가 검토했다"고 쓸 수 있다. 자격 단계 계약과 3차년도 전환 절차는 `docs/original_plan/REVIEWER_QUALIFICATION_POLICY.md`에 있다.

**입력은 이름과 소속만 받는다.** 면허 번호와 자격 확인 방법 칸은 없앴다. 아직 존재하지 않는 값을 받으면 `not_collected` 같은 자리표시자가 남고, 그것이 나중에 자격 확인 근거로 잘못 읽히기 때문이다. 검토자는 이미 과제 참여자로 등록돼 있어 신원과 소속이 연동돼 있다.

`data/original_plan/final_session/op039_external_reviewer_form.html`은 다음 조건을 충족한다.
...
- 입력란은 검토자 성명, 소속, 서명 세 개뿐이다. 셋 다 비어 있다.
- 서명은 제출 직전에 본인이 입력한다. 성명과 다르면 제출이 막힌다.
- `was_ai_draft_reviewer`가 검토자가 직접 체크하는 미선택 항목이다.
- `reviewer_role=project_pharmacist_candidate`, `qualification_stage=pharmacist_candidate`, `relationship_to_project=project_co_researcher`, `independent_of_implementation_team=false`는 사실대로 고정 기록된다.

사전 점검이 이 상태를 실제 렌더링된 DOM으로 확인한다. 실행 결과는 `case_count` 10, `preselected_count` 0, `comment_count` 10, `prefilled_comment_count` 0, `READY`, 종료 코드 0이었다.

백엔드는 제출을 아래 다섯 가지에서 막는다.
...
- 검토자가 등록된 과제 참여자가 아니면 거부한다.
- 소속이 과제 등록 정보와 다르면 거부한다.
- 검토자 이름이 H-003 초안 원장에 있는데 `was_ai_draft_reviewer`를 false로 신고하면 거부한다. 원장은 읽기 전용으로 연다.
- 검토자가 `licensed_pharmacist` 자격을 주장하면 거부한다. 현재 계약상 단계는 `pharmacist_candidate`다.

**등록해도 완료가 아니다.** 완료 기록에 `requires_licensed_reconfirmation: true`가 남고, 예비 약사 검토는 OP-039를 `EXTERNAL` 단계로 올리지 않는다. 판정 데이터는 이번에도 만들지 않았다.

### 10건에서 검토자가 볼 정보

`검토자 판정`은 모든 행에서 **미선택**이다. `시스템 신호`는 구현이 낸 고위험 표시이며 검토자의 결론이 아니다.

| 사례 | 문제와 성분·상호작용 | 근거 위치 | 시스템 신호 | 검토자 판정 |
|---|---|---|---|---|
...
권혁찬 예비 약사는 사례별 근거를 읽고 판정과 의견을 직접 입력한다. 10건 모두 끝난 뒤 이름과 소속이 과제 등록 정보와 같은지 확인하고 서명란에 본인 이름을 입력한 다음 `검토 결과 제출`을 한 번 누른다. 하나라도 `부적절`이면 시스템은 결과를 보존하면서 H-005 완료를 유예한다. 운영 보조자는 판정을 `타당`으로 바꾸지 않는다.
...
3. 3차년도 면허 취득 뒤 같은 10개 사례를 언제 다시 검토할 것인가? 최종 마감이 2027-10이므로 재검토 일정을 그 전에 잡아야 한다.
...
- 생성된 실제 AI 초안 전부를 권혁찬 예비 약사가 승인·수정 승인·반려했고 대기 0건이다.
- 승인 초안만 사용한 학습·평가 1회의 입력 계보와 결과가 있다.
- 권혁찬 예비 약사가 H-002 9개 규칙과 H-005 10개 사례를 직접 판단했다.
- 오너가 H-004 보고서 문체와 H-006 영수증을 직접 승인했다.
- H-005 검토자 이름·소속·서명은 실제 값이며 예비 약사 사전 검토로 기록됐다.
- 운영 영수증과 최종 두 영수증의 서명·hash가 유효하다.
- 최종 감사가 `READY`, `goal_complete: true`, 빈 차단 목록을 반환한다.

어느 조건도 기존 완료 표시, 자동 저장, 미리 선택된 답, AI 의견, 오너의 검토자 대리 판단으로 대신할 수 없다.
