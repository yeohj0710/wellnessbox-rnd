...
### 2026-07-30 15:00~17:30 +09:00 — 검토자 자격 구조를 예비 약사 기준으로 재설정

- **작업 일자·근거:** 2026-07-30. 시작 HEAD `79d0811`; 오너가 알려 준 사실(참여자 2인 모두 2027-01 약사 면허 취득 예정, 최종 마감 2027-10, 2차년도 완료 의무 없음)과 이번 실행의 명령 출력.
- **작업 목적:** 지금까지 저장소는 검토자를 "프로젝트 소속 면허 약사"로 적고 면허 번호를 요구했다. 실제로는 두 사람 모두 아직 면허가 없다. 사실과 다른 표현을 걷어내고, 2차년도 검토를 예비 약사 사전 검토로 기록하도록 구조를 바꾸는 것이 목적이었다.
- **확인한 원자료와 구현 파일:** `src/wellnessbox_rnd/governance/final_session_console.py::register_external_validation`, `reviewer_credentials.py`, `data/original_plan/final_session/external_validation/op039_external_validation.json`의 저장된 자격 값, `data/original_plan/requirements_manifest_v1.json`의 OP-039 단계, 두 trust-root 계약, `human_signoff_checklist.md`의 H-003·H-005 항목.
- **수행한 변경 또는 실험:** 신원 원장에 자격 단계 계약과 등록 참여자 두 명을 넣었다. 자격 검증 모듈을 면허 검사에서 참여자 확인으로 바꾸고, `licensed_pharmacist` 허위 주장 차단과 소속 일치 검사를 넣었다. 검토 화면 입력을 성명·소속·서명 세 개로 줄였다. 콘솔이 예비 약사 검토를 `pharmacist_candidate_preliminary_safety_review`로 기록하고 OP-039를 `EXTERNAL`로 올리지 않도록 막았다. 자격 단계 정책 문서를 새로 쓰고 실행서·체크리스트·운영 절차·요약·OP-039 보고서를 모두 고쳤다.
- **실패와 수정:** 콘솔 시험 두 건이 실패했다. 하나는 픽스처가 등록되지 않은 가상의 "외부약사"를 쓰고 있었고, 다른 하나는 화면에 면허 입력란이 있다고 단언하고 있었다. 둘 다 새 사실에 맞게 고쳤다. 검토 화면 중립성 시험도 면허·자격 칸의 존재를 요구하고 있어 "그 칸이 없어야 한다"로 뒤집었다. 전체 회귀에서 감사 blob 시험이 또 실패했으나 원인은 미커밋 상태였고 커밋 뒤 감사 재실행으로 해소했다.
- **테스트·검증 결과:** `pytest tests/test_reviewer_credentials.py` 30건, `tests/test_op039_reviewer_form_neutrality.py` 7건, `tests/test_final_session_console.py` 27건이 통과했다. 실제 원장 대조에서 권혁찬이 H-003 검토자로 확인돼 `was_ai_draft_reviewer=false` 신고가 차단됐다. 두 trust-root 계약이 모두 0건이라 외부 기관 평가 경로는 여전히 닫혀 있다. 전체 pytest와 사전 점검, OP-120 감사 결과는 이 항목 아래 최종 커밋에 기록했다.
- **그 시점의 미확인 사항:** 3차년도 약사 자격 재검토 일정이 아직 잡히지 않았다. 학습 게이트는 여전히 NO-GO다. 서로 다른 실제 프로필 5건의 전체 경로도 실행되지 않았다.
- **기록 정정:** 저장소가 검토자를 "프로젝트 소속 면허 약사"로 적고 `pharmacist_license_id`를 요구하던 구조는 사실과 달랐다. 과거 기록에 남은 `not_collected`와 `project_owner_attestation`은 그 구조 때문에 생긴 자리표시자다. 과거 저장 기록 자체는 이력으로 보존하고 고치지 않았다.
- **다음 작업:** CGM 기하 blocker 해소, 실제 프로필 5건 실행, 2차년도 최종 세션을 예비 약사 검토로 완료. 3차년도에는 자격 단계만 올려 같은 사례를 다시 검토한다.
- **작성자 / 검토자:** 작성자 Claude Code 작업팀(자동, 실시간 기록); 사람 검토자 `미검토`.
